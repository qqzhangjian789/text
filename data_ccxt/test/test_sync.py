...
path = os.path.dirname(data_ccxt.__file__)
if 'site-packages' in os.path.dirname(data_ccxt.__file__):
    raise Exception("You are running test_async.py/test.py against a globally-installed version of the library! It was previously installed into your site-packages folder by pip or pip3. To ensure testing against the local folder uninstall it first with pip uninstall data_ccxt or pip3 uninstall data_ccxt")
...
def try_all_proxies(exchange, proxies=['']):
    ...
    for num_retries in range(0, max_retries):
        ...
        except (data_ccxt.RequestTimeout, data_ccxt.AuthenticationError, data_ccxt.NotSupported, data_ccxt.DDoSProtection, data_ccxt.ExchangeNotAvailable, data_ccxt.ExchangeError) as e:
...
for id in data_ccxt.exchanges:
    if id == 'theocean':
        continue
    exchange = getattr(data_ccxt, id)
    exchange_config = {'verbose': argv.verbose}
    if sys.version_info[0] < 3:
        exchange_config.update({'enableRateLimit': True})
    if id in config:
        exchange_config = data_ccxt.Exchange.deep_extend(exchange_config, config[id])
